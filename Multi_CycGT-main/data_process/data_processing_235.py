...
if __name__ == '__main__':
    ...
    for train_index, test_index in kf.split(X_stand):
        ...
        X_train, X_val, y_train, y_val = train_test_split(X_train, y_train,
                                                          test_size=0.1,
                                                          random_state=3407)
        pd.DataFrame(X_train,columns=df_X.columns).to_csv('../data/data_splitClassifier_235/X_train{}.csv'.format(i),index=False)
        pd.DataFrame(X_test,columns=df_X.columns).to_csv('../data/data_splitClassifier_235/X_test{}.csv'.format(i),index=False)
        pd.DataFrame(X_val,columns=df_X.columns).to_csv('../data/data_splitClassifier_235/X_val{}.csv'.format(i),index=False)
        pd.DataFrame(y_train,columns=['target']).to_csv('../data/data_splitClassifier_235/y_train{}.csv'.format(i),index=False)
        pd.DataFrame(y_test,columns=['target']).to_csv('../data/data_splitClassifier_235/y_test{}.csv'.format(i),index=False)
        pd.DataFrame(y_val,columns=['target']).to_csv('../data/data_splitClassifier_235/y_val{}.csv'.format(i),index=False)
