...
if __name__ == '__main__':
    ...
    for train_index, test_index in kf.split(X_stand):
        ...
        # pd.DataFrame(X_test, columns=data.columns).to_csv('../data/data_splitClassifier/X_train{}.csv'.format(i),index=False)
        # pd.DataFrame(X_val, columns=data.columns).to_csv('../data/data_splitClassifier/X_train{}.csv'.format(i),index=False)
        X_train_df = pd.DataFrame(X_train,columns=data.columns)
        X_train_df['label'] = y_train
        X_train_df.to_csv('../data/data_splitClassifier/X_train{}.csv'.format(i),index=False)

        X_test_df = pd.DataFrame(X_test,columns=data.columns)
        X_test_df['label'] = y_test
        X_test_df.to_csv('../data/data_splitClassifier/X_test{}.csv'.format(i),index=False)

        X_val_df = pd.DataFrame(X_val,columns=data.columns)
        X_val_df['label'] = y_val
        X_val_df.to_csv('../data/data_splitClassifier/X_val{}.csv'.format(i),index=False)
